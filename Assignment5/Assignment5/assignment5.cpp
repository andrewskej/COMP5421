...
WordsVector remove_duplicates(const WordsVector& words_vector){
    ...
    std::sort(words_vec.begin(), words_vec.end(), [](const std::string & a, const std::string & b) -> bool
    {
        return a < b;
    });
...
bool is_palindrome(const std::string& phrase){
    ...
    bool isPalin = std::equal(buffer.begin(), buffer.begin() + buffer.size() / 2, buffer.rbegin());
...
void multisetUsingMyComparator(const std::vector<std::string>& vec){
    //call multiset but using a customized comparator
...
class Fibo{
    ...
    public:
        ...
        int operator()(){
            //adding one more function just to use recursive...
            //there might be a way to have it lambda recursive, but I won't bother
...
std::vector<int> getnerate_Fibonacci(int n){
    ...
    std::generate_n(fibo.begin(), n, Fibo{n});
    return fibo;
}
